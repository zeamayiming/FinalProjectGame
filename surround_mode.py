...
from zombie_surround import Zombie
...
pygame.display.set_caption("好壞喔！！從四方來襲")
...
# 城堡位置設定為畫面中央
CASTLE_CENTER_X = config.WIDTH // 2
CASTLE_CENTER_Y = config.HEIGHT // 2 - 50
...
castle.rect.centerx = CASTLE_CENTER_X
castle.rect.centery = CASTLE_CENTER_Y
...
def draw_scoreboard():
    lines = get_scoreboard_text("surround")
...
def draw_castle_area():
    # 繪製城堡周圍的綠色草地圓圈
    pygame.draw.circle(screen, config.GRASS_GREEN,
                       (CASTLE_CENTER_X, CASTLE_CENTER_Y), 300)
    # 繪製內圈（禁止種植區域）
    pygame.draw.circle(screen, config.WHITE,
                       (CASTLE_CENTER_X, CASTLE_CENTER_Y), 80)
    # 繪製外圍邊界線
    pygame.draw.circle(screen, config.BLACK,
                       (CASTLE_CENTER_X, CASTLE_CENTER_Y), 300, 2)
    pygame.draw.circle(screen, config.BLACK,
                       (CASTLE_CENTER_X, CASTLE_CENTER_Y), 80, 2)
...
while running:
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEBUTTONDOWN and not game_over:
            ...
            distance_to_castle = math.sqrt(
                (x - CASTLE_CENTER_X)**2 + (y - CASTLE_CENTER_Y)**2)

            # 在草地範圍內（80-300像素）且有足夠分數
            if 80 <= distance_to_castle <= 300 and score >= 3:
                temp_plant = Plant(x, y)
                temp_plant.set_bullet_image(zombie_img)

                # 檢查是否與其他物件重疊
                plant_collides = pygame.sprite.spritecollideany(
                    temp_plant, plants)
                zombie_collides = pygame.sprite.spritecollideany(
                    temp_plant, enemies)
                castle_collides = temp_plant.rect.colliderect(castle.rect)

                if not (plant_collides or zombie_collides or castle_collides):
        ...
        if event.type == pygame.KEYDOWN and game_over and input_active:
            if event.key == pygame.K_RETURN:
                save_score(name_input, score_display.score, "surround")
                input_active = False
    ...
    if not game_over:
        ...
        if now - zombie_timer >= zombie_interval:
            zombie_timer = now
            # 使用 zombie_surround.py 的生成方法
            zombie = Zombie.generate_zombie(zombie_img, mode=2)
            zombie.speed = zombie_speed
            zombie.target_x = CASTLE_CENTER_X
            zombie.target_y = CASTLE_CENTER_Y
        ...
        draw_health_bar(screen, castle.health, 5,
                        castle.rect.centerx - 20, castle.rect.top - 10)
    ...
    # 4. 繪製遊戲元素（按照層次順序）
    all_sprites.draw(screen)  # 繪製城堡和其他精靈
    for plant in plants:  # 繪製植物射程
        draw_range(screen, plant.rect.centerx, plant.rect.centery, plant.range)
    zombie_sprites.draw(screen)  # 繪製殭屍

    # 5. 繪製UI元素（最上層）
    # 左側UI
    pygame.draw.rect(screen, config.WHITE, (0, 0, 100, config.HEIGHT))
    # 底部UI
    pygame.draw.rect(screen, config.WHITE,
                     (100, config.HEIGHT - 150, config.WIDTH - 100, 150))

    # 繪製邊界線
    pygame.draw.line(screen, config.BLACK, (100, 0),
                     (100, config.HEIGHT - 150), 3)  # 左邊界
    pygame.draw.line(screen, config.BLACK, (100, config.HEIGHT - 150),
                     (config.WIDTH, config.HEIGHT - 150), 3)  # 底邊界
    ...
    screen.blit(level_text, (config.WIDTH - 300, 10))
